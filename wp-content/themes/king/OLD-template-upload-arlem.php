<?php /* Template Name: template-upload-arlem */ ?>
<?php
/**
 * Submit ARLEM Page.
 * KB 
	...
	<div id="primary" class="page-content-area">
		<main id="main" class="page-site-main king-submit-arlem">
			...
			<form id="king_posts_form" action="" method="POST" enctype="multipart/form-data">
				<!--Uploaded File -->
				<?php
				if (is_user_logged_in() && ! isset( $_POST['acf']['field_99f5335001eed']) ) {
					$arlem_file_url = sanitize_text_field($_GET["file"]);
					if ($arlem_file_url) {
						$retval = apply_filters( 'upload_arlem_file', $arlem_file_url );
						if ($retval > 0) {
				...
				<!-- Add Category -->
				<span class="info-block"><?php esc_html_e( 'Select one or more categories', 'king' ) ?></span>
				<?php
				$include    = array();
				...
				<!-- LICENCES --> 
				<span class="info-block"><?php esc_html_e( 'Please select a license', 'king' ) ?></span>
				...
				<div class="king-form-group form-licences" >
					...
					<span class="info-block"><?php esc_html_e( 'Add an image to your post. If the uploaded zip contains a file named thumbnail.jpg, or you are adding a video, you do not need to add an image', 'king' ) ?></span>
